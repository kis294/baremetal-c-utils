...
*/
void insert(size_t address, size_t size, uint32_t line,const char* func) {
    // check for null
    if (address == 0) {
    ...
    if (data.total_free_size != data.total_allocated_size) {
        ...
        for (int i=0; i<MAX_ALLOCATIONS; i++) {
            if (data.mem[i].address != 0) {
                printf("Memory leak at in func:%s , line %d: (%lu bytes)\n", 
                    data.mem[i].func_name,
                    data.mem[i].line,
                    data.mem[i].size);
            }
        }
...
void *_malloc(size_t size, uint32_t line,const char* str) {
    void *ptr = malloc(size);

    // insert to memory data
    insert((size_t)ptr, size, line,str);
